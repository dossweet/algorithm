...
不存在3，返回false
#### 分析
看到这道题，我的第一反应是，先找出关键语句<B>每一行都按照从左到右递增的顺序排序，每一列都按照从上到下递增的顺序排序</B>。  
因此解题思路应该是先找出满足要查找的元素大于第一列的哪个元素&&查找的元素小于该行最后一列中的元素条件的行，然后在该行中逐个进行比较就行了。
通过代码实践发现我这种思路对于时间复杂度其实没有什么优化。因为很多行都可能满足上述这个条件。  

<B>这个题其实是一个逻辑题。</B>  
根据我加粗的那段话，就是逻辑的突破口：根据左下/右上移动法进行遍历比较，只需要o(1)的复杂度即可找出要比较的元素是否在数组中。左下角元素的特殊性在于它是第一列里最大的，也是该行最小的。因此和右上角一样最具特殊性。  
按照左下移动法，先判断target与左下角元素（记为lastLeftOne)的关系:  
如果target=lastLeftOne,直接输出。    
如果target<lastLeftOne,上移。  
如果target>lastLeftOne,右移。
这样最多遍历一次数组，即可找出结果。
```java
